...
Sans-IO OpenAI API client for Gleam, ported from the
[async-openai](https://github.com/64bit/async-openai) Rust library.
...
1. Read the corresponding Rust types in the upstream [async-openai](https://github.com/64bit/async-openai) `src/types/<module>/`
...
The upstream Rust source of truth is at
[github.com/64bit/async-openai](https://github.com/64bit/async-openai).
Key locations in that repo:

- Types: `src/types/<module>/`
- Config: `src/config.rs`
- Errors: `src/error.rs`
- API implementations: `src/<module>.rs`
- Examples: `examples/`

Example fixtures (e.g. PDFs for vector-store-retrieval) are copied into
`dev/example/input/<example-name>/` so this repo stays self-contained.
